...
int main() {
  ...
  for (int64_t i = 0; i < Q; i++) {
    ...
    std::vector<double> projection = {coefficient * p.at(0),
                                      coefficient * p.at(1)};

    std::vector<double> reflect = {projection.at(0) - q.at(0),
                                   projection.at(1) - q.at(1)};

    std::cout << std::fixed << std::setprecision(10)
              << x.at(i) + 2 * reflect.at(0) << " "
              << y.at(i) + 2 * reflect.at(1) << std::endl;
  }
