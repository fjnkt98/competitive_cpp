...
S mapping(int64_t f, S s) { return {s.value + f * s.size, s.size}; }

int64_t composition(int64_t f, int64_t g) { return f + g; }
...
int main() {
  ...
  LazySegmentTree<S, op, e, int64_t, mapping, composition, id> seg(
      std::vector<S>(N, {0, 1}));
  ...
  for (auto q : query) {
    ...
    if (c == 0) {
      ...
      std::cout << seg.prod(s, t + 1).value << std::endl;
    }
  }
